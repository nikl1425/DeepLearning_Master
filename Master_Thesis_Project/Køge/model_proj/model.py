from tensorflow.keras.layers import Conv2D, MaxPooling2D, Dropout, Flatten, Activation, Input, concatenate, Dense, Reshape
...
def get_covn_base(input_layer, img_shape):
    input = Conv2D(128, (4,4), input_shape=img_shape)(input_layer)
    acti01 = Activation('relu')(input)
    pool01 = MaxPooling2D((2, 2))(acti01)
    covn02 = Conv2D(64, (4, 4), kernel_regularizer=l2(1e-5))(pool01)
    acti02 = Activation('relu')(covn02)
    pool02 = MaxPooling2D(2, 2)(acti02)
    covn03 = Conv2D(32, (4, 4), kernel_regularizer=l2(1e-5))(pool02)
    acti03 = Activation('relu')(covn03)
    pool03 = MaxPooling2D(pool_size=(2,2), padding='same')(acti03)
    covn04 = Conv2D(16, (4, 4), kernel_regularizer=l2(1e-5))(pool03)
    acti04 = Activation('relu')(covn04)
    pool04 = MaxPooling2D(pool_size=(2,2), padding='same')(acti04)
    flatten = Flatten()(pool04)
    return flatten

def get_small_covn_base(input_layer, img_shape):
    covn01 = Conv2D(32, (4,4), padding='same', kernel_regularizer=l2(1e-5), input_shape=img_shape)(input_layer)
    acti01 = Activation('relu')(covn01)
    pool01 = MaxPooling2D((4, 4))(acti01)
    covn02 = Conv2D(16, (4, 4), kernel_regularizer=l2(1e-5))(pool01)
    acti02 = Activation('relu')(covn02)
    pool02 = MaxPooling2D(4, 4)(acti02)
    flatten = Flatten()(pool02)
    return flatten
...
def get_shallow_cnn(img_shape):
    ...
    fully_connected_dense_big = Dense(256, activation='relu', kernel_regularizer=l2(1e-5))(concat_feature_layer)
...
def get_shallow_three_input_cnn(img_shape):
    ...
    model_three = get_covn_base(model_three_input, img_shape)
...
def get_small_three_input_cnn(img_shape):
    ...
    model_three_input = Input(shape=img_shape)
    model_three = get_small_covn_base(model_three_input, img_shape)
...
def get_vgg16_resnet152(img_shape, trainable=False):
    ...
    flatten_layer = Flatten()(concat_feature_layer)
    concat_feature_layer_2 = concatenate([flatten_layer, model_one])
    fully_connected_dense_big = Dense(1024, activation='relu')(concat_feature_layer_2)
    dropout_one = Dropout(0.5)(fully_connected_dense_big)
    fully_connected_dense_small = Dense(512, activation='relu')(dropout_one)
    dropout_two = Dropout(0.5)(fully_connected_dense_small)
    output = Dense(3, activation='softmax')(dropout_two)

    model = Model(
        inputs=[resnet.input, vgg16.input, model_one_input],
        outputs=output
    )
